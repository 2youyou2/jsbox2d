...
inline void b2DynamicTree::RayCast(T* callback, const b2RayCastInput& input) const
{
	b2Vec2 p1 = input.p1;
	b2Vec2 p2 = input.p2;
	b2Vec2 r = b2Vec2::Subtract(p2, p1);
	b2Assert(r.LengthSquared() > 0.0);
	r.Normalize();

	// v is perpendicular to the segment.
	b2Vec2 v = b2Cross_f_v2(1.0, r);
	b2Vec2 abs_v = b2Abs_v2(v);
	...
	while (stack.GetCount() > 0)
	{
		...
		float32 separation = b2Abs(b2Dot_v2_v2(v, b2Vec2::Subtract(p1, c))) - b2Dot_v2_v2(abs_v, h);
		if (separation > 0.0)
		{
			continue;
		}
