...
struct b2SeparationFunction
{
		...
		float32 t1)
	{
		...
		if (count == 1)
		{
			...
			b2Vec2 localPointA = this->m_proxyA->GetVertex(cache->indexA[0]);
	...
	float32 FindMinSeparation(int32* indexA, int32* indexB, float32 t) const
	{
		...
		case b2SeparationFunction::e_points:
			{
				b2Vec2 axisA = b2MulT_r_v2(xfA.q,  this->m_axis);
				b2Vec2 axisB = b2MulT_r_v2(xfB.q, this->m_axis.Negate());

				*indexA = this->m_proxyA->GetSupport(axisA);
				*indexB = this->m_proxyB->GetSupport(axisB);

				b2Vec2 localPointA = this->m_proxyA->GetVertex(*indexA);
				b2Vec2 localPointB = this->m_proxyB->GetVertex(*indexB);

				b2Vec2 pointA = b2Mul_t_v2(xfA, localPointA);
				b2Vec2 pointB = b2Mul_t_v2(xfB, localPointB);

				float32 separation = b2Dot_v2_v2(b2Vec2::Subtract(pointB, pointA), this->m_axis);
				return separation;
			}
		...
		case b2SeparationFunction::e_faceA:
			{
				...
				b2Vec2 axisB = b2MulT_r_v2(xfB.q, normal.Negate());
				...
				*indexB = this->m_proxyB->GetSupport(axisB);
				...
				float32 separation = b2Dot_v2_v2(b2Vec2::Subtract(pointB, pointA), normal);
				return separation;
			}
		...
		case b2SeparationFunction::e_faceB:
			{
				...
				b2Vec2 axisA = b2MulT_r_v2(xfA.q, normal.Negate());
				...
				*indexA = this->m_proxyA->GetSupport(axisA);
				...
				float32 separation = b2Dot_v2_v2(b2Vec2::Subtract(pointA, pointB), normal);
				return separation;
			}
...
void b2TimeOfImpact(b2TOIOutput* output, const b2TOIInput* input)
{
	...
	float32 target = b2Max(b2_linearSlop, totalRadius - 3.0 * b2_linearSlop);
