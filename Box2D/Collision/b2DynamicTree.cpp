...
b2DynamicTree::b2DynamicTree()
{
	...
	this->m_nodeCount = 0;
	this->m_nodes = (b2TreeNode*)b2Alloc(this->m_nodeCapacity * sizeof(b2TreeNode));
	memset(this->m_nodes, 0, this->m_nodeCapacity * sizeof(b2TreeNode));

	// Build a linked list for the free list.
	for (int32 i = 0; i < this->m_nodeCapacity - 1; ++i)
	{
...
int32 b2DynamicTree::AllocateNode()
{
	...
	if (this->m_freeList == b2_nullNode)
	{
		...
		for (int32 i = this->m_nodeCount; i < this->m_nodeCapacity - 1; ++i)
		{
			this->m_nodes[i].parent = i + 1;
			this->m_nodes[i].height = -1;
		}
...
void b2DynamicTree::InsertLeaf(int32 leaf)
{
	...
	while (this->m_nodes[index].IsLeaf() == false)
	{
		...
		float32 cost = 2.0 * combinedArea;

		// Minimum cost of pushing the leaf further down the tree
		float32 inheritanceCost = 2.0 * (combinedArea - area);
...
float32 b2DynamicTree::GetAreaRatio() const
{
	...
	const b2TreeNode* root = this->m_nodes + this->m_root;
	float32 rootArea = root->aabb.GetPerimeter();

	float32 totalArea = 0.0;
	for (int32 i = 0; i < this->m_nodeCapacity; ++i)
	{
...
{
	int32 height = ComputeHeight(this->m_root);
	return height;
}
...
void b2DynamicTree::Validate() const
{
	...
	int32 freeCount = 0;
	int32 freeIndex = this->m_freeList;
...
{
	int32* nodes = (int32*)b2Alloc(this->m_nodeCount * sizeof(int32));
	int32 count = 0;
