	...
	b2ContactListener* listener)
{
	...
	this->m_bodies = (b2Body**)this->m_allocator->Allocate(bodyCapacity * sizeof(b2Body*));
	this->m_contacts = (b2Contact**)this->m_allocator->Allocate(contactCapacity	 * sizeof(b2Contact*));
	this->m_joints = (b2Joint**)this->m_allocator->Allocate(jointCapacity * sizeof(b2Joint*));

	this->m_velocities = (b2Velocity*)this->m_allocator->Allocate(this->m_bodyCapacity * sizeof(b2Velocity));
	this->m_positions = (b2Position*)this->m_allocator->Allocate(this->m_bodyCapacity * sizeof(b2Position));
}
...
void b2Island::Solve(b2Profile* profile, const b2TimeStep& step, const b2Vec2& gravity, bool allowSleep)
{
	...
	for (int32 i = 0; i < this->m_jointCount; ++i)
	{
	...
	{
		for (int32 j = 0; j < this->m_jointCount; ++j)
		{
	...
	for (int32 i = 0; i < this->m_bodyCount; ++i)
	{
		b2Vec2 c = this->m_positions[i].c;
		float32 a = this->m_positions[i].a;
		b2Vec2 v = this->m_velocities[i].v;
		float32 w = this->m_velocities[i].w;

		// Check for large velocities
		b2Vec2 translation = b2Vec2::Multiply(h, v);
	...
	for (int32 i = 0; i < step.positionIterations; ++i)
	{
		...
		for (int32 i = 0; i < this->m_jointCount; ++i)
		{
			bool jointOkay = this->m_joints[i]->SolvePositionConstraints(solverData);
