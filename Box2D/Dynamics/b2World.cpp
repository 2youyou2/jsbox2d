...
b2World::b2World(const b2Vec2& gravity)
{
	...
	memset(&this->m_profile, 0, sizeof(b2Profile));
}

b2World::~b2World()
{
	// Some shapes allocate using b2Alloc.
	b2Body* b = this->m_bodyList;
	while (b)
	{
...
void b2World::SetAllowSleeping(bool flag)
{
	...
	{
		for (b2Body* b = this->m_bodyList; b; b = b->m_next)
		{
			b->SetAwake(true);
		}
	}
...
void b2World::Solve(const b2TimeStep& step)
{
	...
	b2Island island(this->m_bodyCount,
	...
	for (b2Contact* c = this->m_contactManager.m_contactList; c; c = c->m_next)
	{
		c->m_flags &= ~b2Contact::e_islandFlag;
	}
	for (b2Joint* j = this->m_jointList; j; j = j->m_next)
	{
		j->m_islandFlag = false;
	}

	// Build and simulate all awake islands.
	int32 stackSize = this->m_bodyCount;
	b2Body** stack = (b2Body**)this->m_stackAllocator.Allocate(stackSize * sizeof(b2Body*));
	for (b2Body* seed = this->m_bodyList; seed; seed = seed->m_next)
	{
	...
	for (;;)
	{
		...
		for (b2Contact* c = this->m_contactManager.m_contactList; c; c = c->m_next)
		{
			...
			else
			{
				...
				bool activeA = bA->IsAwake() && typeA != b2Body::b2_staticBody;
				bool activeB = bB->IsAwake() && typeB != b2Body::b2_staticBody;
				...
				bool collideA = bA->IsBullet() || typeA != b2Body::b2_dynamicBody;
				bool collideB = bB->IsBullet() || typeB != b2Body::b2_dynamicBody;
...
{
	for (b2Body* body = this->m_bodyList; body; body = body->GetNext())
	{
...
struct b2WorldQueryWrapper
{
	...
	{
		b2FixtureProxy* proxy = (b2FixtureProxy*)this->broadPhase->GetUserData(proxyId);
...
struct b2WorldRayCastWrapper
{
	...
	{
		void* userData = this->broadPhase->GetUserData(proxyId);
		...
		if (hit)
		{
			...
			b2Vec2 point = b2Vec2::Add(b2Vec2::Multiply((1.0 - fraction), input.p1), b2Vec2::Multiply(fraction, input.p2));
	...
	case b2Shape::e_circle:
		{
			...
			b2Vec2 axis = b2Mul_r_v2(xf.q, b2Vec2(1.0, 0.0));
	...
	case b2Shape::e_edge:
		{
			...
			b2Vec2 v1 = b2Mul_t_v2(xf, edge->m_vertex1);
			b2Vec2 v2 = b2Mul_t_v2(xf, edge->m_vertex2);
	...
	case b2Shape::e_chain:
		{
			...
			b2Vec2 v1 = b2Mul_t_v2(xf, vertices[0]);
...
void b2World::DrawDebugData()
{
	...
	{
		for (b2Joint* j = this->m_jointList; j; j = j->GetNext())
		{
			DrawJoint(j);
		}
	}
	...
	{
		b2Color color(0.3, 0.9, 0.9);
		for (b2Contact* c = this->m_contactManager.m_contactList; c; c = c->GetNext())
		{
	...
	{
		b2Color color(0.9, 0.3, 0.9);
		b2BroadPhase* bp = &this->m_contactManager.m_broadPhase;
...
void b2World::Dump()
{
	...
	for (b2Joint* j = this->m_jointList; j; j = j->m_next)
	{
		if (j->m_type == b2Joint::e_gearJoint)
		{
			continue;
		}
