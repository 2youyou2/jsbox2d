...
void b2Fixture::Create(b2BlockAllocator* allocator, b2Body* body, const b2FixtureDef* def)
{
	...
	this->m_shape = def->shape->Clone(allocator);

	// Reserve proxy space
	int32 childCount = this->m_shape->GetChildCount();
	this->m_proxies = (b2FixtureProxy*)allocator->Allocate(childCount * sizeof(b2FixtureProxy));
...
void b2Fixture::Destroy(b2BlockAllocator* allocator)
{
	...
	case b2Shape::e_circle:
		{
			b2CircleShape* s = (b2CircleShape*)this->m_shape;
			s->~b2CircleShape();
	...
	case b2Shape::e_edge:
		{
			b2EdgeShape* s = (b2EdgeShape*)this->m_shape;
			s->~b2EdgeShape();
	...
	case b2Shape::e_polygon:
		{
			b2PolygonShape* s = (b2PolygonShape*)this->m_shape;
			s->~b2PolygonShape();
	...
	case b2Shape::e_chain:
		{
			b2ChainShape* s = (b2ChainShape*)this->m_shape;
			s->~b2ChainShape();
...
void b2Fixture::CreateProxies(b2BroadPhase* broadPhase, const b2Transform& xf)
{
	...
	for (int32 i = 0; i < this->m_proxyCount; ++i)
	{
		b2FixtureProxy* proxy = this->m_proxies + i;
...
void b2Fixture::Synchronize(b2BroadPhase* broadPhase, const b2Transform& transform1, const b2Transform& transform2)
{
	...
	for (int32 i = 0; i < this->m_proxyCount; ++i)
	{
		...
		b2Vec2 displacement = b2Vec2::Subtract(transform2.p, transform1.p);
...
void b2Fixture::Refilter()
{
	...
	b2ContactEdge* edge = this->m_body->GetContactList();
	while (edge)
	{
