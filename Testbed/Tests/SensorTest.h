...
public:
	...
	void Step(Settings* settings)
	{
		...
		for (int32 i = 0; i < e_count; ++i)
		{
			...
			b2Body* ground = this->m_sensor->GetBody();

			b2CircleShape* circle = (b2CircleShape*)this->m_sensor->GetShape();
			b2Vec2 center = ground->GetWorldPoint(circle->m_p);

			b2Vec2 position = body->GetPosition();

			b2Vec2 d = b2Vec2::Subtract(center, position);
			...
			d.Normalize();
			b2Vec2 F = b2Vec2::Multiply(100.0, d);
