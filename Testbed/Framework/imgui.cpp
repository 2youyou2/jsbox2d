...
bool imguiItem(const char* text, bool enabled)
{
	...
	if (isHot(id))
		AddGfxCmdRoundedRect((float)x, (float)y, (float)w, (float)h, 2.0, SetRGBA(255, 196, 0, isActive(id) ? 196 : 96));
...
bool imguiSlider(const char* text, float* val, float vmin, float vmax, float vinc, bool enabled)
{
	...
	AddGfxCmdRoundedRect((float)x, (float)y, (float)w, (float)h, 4.0, SetRGBA(0, 0, 0, 128));
	...
	if (isActive(id))
	{
		...
		if (s_state.dragX != s_state.mx)
		{
			...
			*val = floorf(*val / vinc + 0.5)*vinc; // Snap to vinc
			m = (int)(u * range);
			valChanged = true;
		}
	}
	...
	if (isActive(id))
		AddGfxCmdRoundedRect((float)(x + m), (float)y, (float)SLIDER_MARKER_WIDTH, (float)SLIDER_HEIGHT, 4.0, SetRGBA(255, 255, 255, 255));
	else
		AddGfxCmdRoundedRect((float)(x + m), (float)y, (float)SLIDER_MARKER_WIDTH, (float)SLIDER_HEIGHT, 4.0, isHot(id) ? SetRGBA(255, 196, 0, 128) : SetRGBA(255, 255, 255, 64));
...
bool imguiSlider(const char* text, int* val, int vmin, int vmax, int vinc, bool enabled)
{
	...
	if (isActive(id))
	{
		...
		if (s_state.dragX != s_state.mx)
		{
			...
			*val = int(floorf(*val / float(vinc) + 0.5))*vinc; // Snap to vinc
			m = (int)(u * range);
			valChanged = true;
		}
	}
