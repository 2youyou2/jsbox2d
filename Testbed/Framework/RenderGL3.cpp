...
void AddGfxCmdRect(float x, float y, float w, float h, unsigned int color)
{
	...
	cmd.flags = 0;
	cmd.col = color;
	cmd.rect.x = (short)(x*8.0);
	cmd.rect.y = (short)(y*8.0);
	cmd.rect.w = (short)(w*8.0);
	cmd.rect.h = (short)(h*8.0);
	cmd.rect.r = 0;
}
...
void AddGfxCmdLine(float x0, float y0, float x1, float y1, float r, unsigned int color)
{
	...
	cmd.flags = 0;
	cmd.col = color;
	cmd.line.x0 = (short)(x0*8.0);
	cmd.line.y0 = (short)(y0*8.0);
	cmd.line.x1 = (short)(x1*8.0);
	cmd.line.y1 = (short)(y1*8.0);
	cmd.line.r = (short)(r*8.0);
}
...
static void sDrawPolygon(const float* coords, unsigned numCoords, float r, unsigned int col)
{
	...
	for (unsigned i = 0, j = numCoords - 1; i < numCoords; j = i++)
	{
		...
		float dmx = (dlx0 + dlx1) * 0.5;
		float dmy = (dly0 + dly1) * 0.5;
...
void RenderGLFlush(int width, int height)
{
	...
	for (int i = 0; i < nq; ++i)
	{
		...
		if (cmd.type == GFXCMD_RECT)
		{
			if (cmd.rect.r == 0)
			{
				sDrawRect((float)cmd.rect.x*s + 0.5, (float)cmd.rect.y*s + 0.5,
						 (float)cmd.rect.w*s - 1, (float)cmd.rect.h*s - 1,
						 1.0, cmd.col);
			}
			else
			{
				sDrawRoundedRect((float)cmd.rect.x*s + 0.5, (float)cmd.rect.y*s + 0.5,
								(float)cmd.rect.w*s - 1, (float)cmd.rect.h*s - 1,
								(float)cmd.rect.r*s, 1.0, cmd.col);
			}
		}
		else if (cmd.type == GFXCMD_LINE)
		{
			sRenderLine(cmd.line.x0*s, cmd.line.y0*s, cmd.line.x1*s, cmd.line.y1*s, cmd.line.r*s, 1.0, cmd.col);
		}
		else if (cmd.type == GFXCMD_TRIANGLE)
		{
			if (cmd.flags == 1)
			{
				...
				{
					(float)cmd.rect.x*s + 0.5, (float)cmd.rect.y*s + 0.5,
					(float)cmd.rect.x*s + 0.5 + (float)cmd.rect.w*s - 1, (float)cmd.rect.y*s + 0.5 + (float)cmd.rect.h*s / 2 - 0.5,
					(float)cmd.rect.x*s + 0.5, (float)cmd.rect.y*s + 0.5 + (float)cmd.rect.h*s - 1,
				};
				sDrawPolygon(verts, 3, 1.0, cmd.col);
			}
			if (cmd.flags == 2)
			{
				const float verts[3 * 2] =
				{
					(float)cmd.rect.x*s + 0.5, (float)cmd.rect.y*s + 0.5 + (float)cmd.rect.h*s - 1,
					(float)cmd.rect.x*s + 0.5 + (float)cmd.rect.w*s / 2 - 0.5, (float)cmd.rect.y*s + 0.5,
					(float)cmd.rect.x*s + 0.5 + (float)cmd.rect.w*s - 1, (float)cmd.rect.y*s + 0.5 + (float)cmd.rect.h*s - 1,
				};
				sDrawPolygon(verts, 3, 1.0, cmd.col);
			}
		}
